...
const next = document.getElementById("nextJoke");
...
const showJoke = ()=>{
    let randArr = Math.floor(Math.random() * (21 - 0) + 0);
   
    joke.innerHTML="";
    const p = document.createElement("p");
    const text = `"${jokeArr[randArr]}"`;
    p.innerText = text;
    joke.appendChild(p);
}


window.addEventListener("load",showJoke);

next.addEventListener("click",showJoke);
